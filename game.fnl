...
(fn try-move [dt]
  (if (love.keyboard.isDown "a")
      ...
      (when (< player.x (- (love.graphics.getWidth) 18))
        (tset player "x" (+ player.x (* player.speed dt))))
      ...
      (when (< player.y (- (love.graphics.getHeight) 11))
        (tset player "y" (+ player.y (* player.speed dt))))))
...
(fn love.load []
  (tset player.anims "run-forward" (make-anim (love.graphics.newImage "assets/player-run-forward.png") 18 11 1))
